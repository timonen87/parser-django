...
class CheckEmail(APIView):
    def post(self, request, format=None):
        ...
        try:
            ...
            return Response(
                {
                    "email": "Пользователя с таким адресом эл.почты не существует или некорректно введён адрес."
                },
...
class CheckToken(APIView):
    def post(self, request, format=None):
        try:
            ...
            return Response(
                {"token": "Токен введён не правильно."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ChangePasswordView(viewsets.ModelViewSet):
    ...
    def change_password(self, request, pk=None):
        ...
        try:
            ...
            print(request.data.get("password_1") == request.data.get("password_2"))
            if request.data.get("password_1") == request.data.get("password_2"):
                if is_password_good(request.data.get("password_1")) == True:
                    ...
                    return Response(
                        {"password_1": message, "password_2": message},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            else:
                return Response(
                    {
                        "password_1": "Пароли не совпадают.",
                        "password_2": "Пароли не совпадают.",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except User.DoesNotExist:
